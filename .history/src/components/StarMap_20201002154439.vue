...
export default {
  ...
  watch: {
    text1: function (val) {
      ...
      while (x == 1) {         
          if ((element.offsetHeight < 60) || (element.offsetWidth < element.scrollWidth)) {
              y--;
               element.style.fontSize = parseInt(y) + 'px';
          } else {
              console.log((element.offsetHeight < 60), (element.offsetHeight < element.scrollHeight) || (element.offsetWidth < element.scrollWidth));
              x = 0;
          }
      }
