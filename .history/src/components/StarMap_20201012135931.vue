  ...
  <div class="okvir">
    <div :class="{'star-map': true, [selected_type]: true, [color_mapped]: true, smallFont: smallFont}">
      ...
      <div :class='{"map": true, "circle": extra_design == "krog", "circle2": extra_design == "krog2", "square": extra_design == "kvadrat"}'>
        ...
        <div id="celestial-map"></div> <!-- // v-show="selected_type === 'normal'" -->
        <!-- <img :src="`/img/${color ? color.toLowerCase() : ''}.png`" alt="" v-show="selected_type === 'extra'" class="fancy"> -->
        <div v-html="genereatedConstelations" v-show="selected_type === 'extra'" class="svg"  id="svg_wrap" alt=""></div>
...
export default {
  ...
  methods: {
    ...
    setTimeout(()=> {
      ...
      console.log("can", can);
      can.classList.add("generate");
      map.classList.add("generate");
      ...
      Celestial.exportSVG((e) => {
        ...
        let json = parse(e).then(json => {
         ...
         let svgParts = [];
                      ...
                      for (let [i, chld] of grp.children.entries()) {
                        ...
                        if ("attributes" in c) {
                          if ("d" in c.attributes) {
                            ...
                            let node = rc.path(c.attributes.d, { roughness: .6, stroke: "white", bowing: 0, strokeWidth: 0.5});
          ...
          let result = '<svg id="svg_stars" title="D3-Celestial" version="1.1" encoding="UTF-8" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" xmlns:sodipodi="http://sodipodi.sourceforge.net/DTD/sodipodi-0.dtd" xmlns:inkscape="http://www.inkscape.org/namespaces/inkscape">';
      ...
      this.config.background.fill = "rgba(255,255,255,1)";
      this.config.background.stroke = "rgba(255,255,255,1)";
