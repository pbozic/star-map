<template>
  <div class="star-map">
    ...
    <div class="text-area">
        <div v-resize-text class="text" :style="{'font-size': font_size1}" ref="posvetilo">
          {{text1 || "Napis 1"}}
        </div>

        <div v-resize-text class="text" :style="{'font-size': font_size2}" ref="malo">
          {{text2 || "Napis 2"}}
        </div>
        <div class="datum">
         {{dateText || "Ljubljna, 01.01.2020"}}
        </div>
    </div>
...
export default {
  ...
  methods: {
    fontSize1(el) {
      ...
      let element = this.$refs.posvetilo;
       console.log("e1", element, element.clientWidt)
        const size =
          Math.max(
            Math.min(
              element.clientWidth / (1 * 10),
     ...
     fontSize2(el) {
      let minFontSize= "16px";
      let maxFontSize= "50px";
      let element = this.$refs.malo;
      console.log("e2", element, element.clientWidth)
        const size =
          Math.max(
            Math.min(
              element.clientWidth / (1 * 10),
