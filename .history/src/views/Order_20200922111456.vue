            ...
            <div class="tab-content" id="myTabContent">
              ...
              <div :class='["tab-pane fade", {"show active": selected_tab === "izgled"}]' id="izgled" role="tabpanel" aria-labelledby="izgled-tab">
                ...
                <div class="row" v-show="selected_product == null">
                  <div class="col">
                    <div class="row">
                      <div :class="{'col-sm-3 vrsta': true, 'active': selected_product ? selected_product.id === product.id ? true : false : false }" v-for="product of products" :key="product.id" @click="selectProduct(product)">
                ...
                <div class="row" v-if="selected_product != null">
                  ...
                  <div class="col">
                    <div class="row">
                      <div :class="{'col-sm-3 barva': true, 'active': selected_color === color }" v-for="(color) of selected_product_variations.colors" :key="color" @click="selected_product_varaint = selected_product_variations.map[color]; selected_color = color">
...
export default {
  ...
  methods: {
    async getLocation() {
      let resp = await axios.get(`https://maps.googleapis.com/maps/api/place/autocomplete/json?input=${this.naslov}&key=${this.api_key}`);
    ...
    async search(input) {
     let resp = await axios.get(`https://maps.googleapis.com/maps/api/place/autocomplete/json?input=${input}&key=${this.api_key}`);
