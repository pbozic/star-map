            ...
            <div class="tab-content" id="myTabContent">
              ...
              <div :class='["tab-pane fade", {"show active": selected_tab === "izgled"}]' id="izgled" role="tabpanel" aria-labelledby="izgled-tab">
                ...
                <div class="row" v-if="selected_product == null">
                  <div class="col">
                    <div class="row">
                      <div class="col vrsta umetniska" v-for="product of products" :key="product.id" @click="selected_product = product">
                ...
                <div class="row" v-if="selected_product != null">
                  ...
                  <div class="col">
                    <div class="row">
                      <div class="col vrsta umetniska" v-for="color of selected_product_variations.colors" :key="color" @click="selected_product_varaint = selected_product_variations.map[color]">
                        <img :src="selected_product_variations.map[color].selectedImage" alt="">
                        <div class="overlay">
                          <span>{{selected_product_variations.map[color].title}}</span>
                        </div>
                      </div>
                    </div>
                  </div>
                  <!-- DETAJLI -->
                  <div class="row" v-if="selected_product.title == 'Umetniška zvezdna karta'">
                    <div class="col">
                      UMETNIŠKA (detajli)
                    </div>
                  </div>
  ...
  methods: {
    ...
    getVariantMap(product) {
      ...
      for (let v of variants) {
        let title = v.title;
        let t_split = title.spit(" / ");
        colors.push(t_split[0])
        sizes.push(t_split[1])
        if (map[t_split[0]] == null) {
          map[t_split[0]] = v;
        }
